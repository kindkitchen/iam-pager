...
/** Static explanation of the product boundary. */
export default define.page(function SiteAbout({ state, url }) {
  const session = state.request_context.session;
  return (
    <SiteEditorialView
      navigation={site_navigation_presenter.present(session, url)}
      breadcrumb={site_breadcrumb_presenter.present({ kind: "about" })}
      editorial={site_editorial_presenter.present("about", session)}
    />
  );
});
